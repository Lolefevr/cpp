...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/15 12:23:20 by lolefevr          #+#    #+#             */
/*   Updated: 2023/10/15 15:28:29 by lolefevr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Convert::Convert(char *av)
{
	...
	else if ((isdigit(argv[0])) || (argv[0] == '-' && isdigit(argv[1])))
	{
		...
		{
			this->num_float = temp;
			ss << std::fixed << std::setprecision(3) << this->num_float << "f";
			this->std_float = ss.str();
			ss.str(std::string());
		}
		...
		else
		{
			...
			ss << std::fixed << std::setprecision(3) << this->num_double;
			this->std_double = ss.str();
			ss.str(std::string());
		}
	}
